...
const placeholder = JSON.parse(await readFile(new URL('../docs/PRODUCTION_0001_0008_APPLICATION_OWNER_RELATION_EVIDENCE.json', import.meta.url), 'utf8'));
assert.equal(placeholder.result, 'NOT_EVALUATED');
assert.equal(placeholder.connectionAttemptCount, 0);
assert.equal(placeholder.sourceCommit, null);
const placeholderText = await readFile(new URL('../docs/PRODUCTION_0001_0008_APPLICATION_OWNER_RELATION_EVIDENCE.json', import.meta.url), 'utf8');
const placeholderHash = createHash('sha256').update(placeholderText).digest('hex');
const placeholderCompanion = await readFile(new URL('../docs/PRODUCTION_0001_0008_APPLICATION_OWNER_RELATION_EVIDENCE.sha256', import.meta.url), 'utf8');
assert.equal(placeholderCompanion, `${placeholderHash}  PRODUCTION_0001_0008_APPLICATION_OWNER_RELATION_EVIDENCE.json\n`);
