...
const missing = inventory.migrations.filter(item => item.sourceStatus === 'MISSING_TRACKED_SOURCE');
assert.deepEqual(missing.map(item => item.version), ['0010']);
assert.equal(missing[0].checksum, null);
assert.equal(missing[0].file, null);

const inventoryResult = await validateExpectedInventory(inventory);
assert.equal(inventoryResult.status, 'BLOCKED');
assert.equal(inventoryResult.trackedCount, 21);
assert.deepEqual(inventoryResult.missingVersions, ['0010']);
assert.ok(inventoryResult.failures.includes('MISSING_TRACKED_SOURCE:0010'));
...
assert.deepEqual(dryRun.stopReasons, ['MIGRATION_MISSING', 'EVIDENCE_INCOMPLETE']);
