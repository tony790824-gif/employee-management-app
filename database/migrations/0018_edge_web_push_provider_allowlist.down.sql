...
BEGIN
  IF EXISTS (
    SELECT 1
     ...
     WHERE endpoint ~ '^https://([a-z0-9-]+\.)*notify\.windows\.com/'
  ) THEN
    RAISE EXCEPTION '0018 rollback requires Edge subscriptions to be revoked and removed first';
  END IF;
  ...
  IF function_body IS NULL
     OR position(new_register_check IN function_body) = 0
     OR position(unregister_replacement IN function_body) = 0
     OR position(test_replacement IN function_body) = 0 THEN
    RAISE EXCEPTION '0018 expected Edge push validation source was not found';
  END IF;

  function_body := replace(function_body, new_register_check, old_register_check);
  function_body := replace(function_body, unregister_replacement, unregister_target);
  function_body := replace(function_body, test_replacement, test_target);
