...
const { Client } = pg;
const VERSION = '0013';
const NAME = 'time_off_requests';
const LOCK_NAME = 'banke-staging-time-off-requests-v1';
const ROLLBACK_CONFIRMATION = 'ROLLBACK_BANKE_STAGING_TIME_OFF_REQUESTS';
const REQUIRED_VERSIONS = Object.freeze([
  '0001', '0002', '0003', '0004', '0005', '0006', '0007', '0008', '0011', '0012'
]);
const UP_FILE = new URL('./migrations/0013_time_off_requests.up.sql', import.meta.url);
const DOWN_FILE = new URL('./migrations/0013_time_off_requests.down.sql', import.meta.url);

function stagingConfig(env = process.env) {
  const config = databaseTargetConfig(env);
  if (config.environment !== 'staging') {
    throw new Error('0013 time_off_requests can only be managed with BANK_ENV=staging.');
  }
  return config;
}
...
async function inspect(client, checksum) {
  ...
  if (current && (current.name !== NAME || current.checksum !== checksum)) {
    throw new Error('Migration 0013 ledger entry does not match the approved source checksum.');
  }

  const objects = (await client.query(
    `SELECT
       to_regclass('public.time_off_requests') IS NOT NULL AS request_table,
       to_regclass('public.time_off_request_dates') IS NOT NULL AS date_table,
       to_regprocedure('app_private.api_list_time_off_requests(text,text,text)') IS NOT NULL AS read_function,
       to_regprocedure('app_private.api_execute_time_off_command(text,text,text,text,jsonb,text,text,text)') IS NOT NULL AS command_function`
  )).rows[0];
  const expectedObjectsExist = Object.values(objects).every(Boolean);
  if (Boolean(current) !== expectedObjectsExist) {
    throw new Error('Migration 0013 ledger and database objects are inconsistent.');
  }

  let forcedRls = 0;
  let tenantPolicies = 0;
  if (current) {
    forcedRls = Number((await client.query(
      `SELECT count(*)::integer AS count
         FROM pg_class relation
         JOIN pg_namespace namespace ON namespace.oid = relation.relnamespace
        WHERE namespace.nspname = 'public'
          AND relation.relname IN ('time_off_requests', 'time_off_request_dates')
          AND relation.relrowsecurity
          AND relation.relforcerowsecurity`
    )).rows[0].count);
    tenantPolicies = Number((await client.query(
      `SELECT count(*)::integer AS count
         FROM pg_policies
        WHERE schemaname = 'public'
          AND tablename IN ('time_off_requests', 'time_off_request_dates')`
    )).rows[0].count);
    if (forcedRls !== 2 || tenantPolicies !== 2) {
      throw new Error('Migration 0013 FORCE RLS or tenant policies are incomplete.');
    }
  }
...
export async function manageStagingTimeOffRequests(client, {
  command = 'status',
  allowRollback = false
} = {}) {
  ...
  if (command === 'down' && !allowRollback) {
    throw new Error('Staging 0013 rollback requires explicit confirmation.');
  }
...
async function main() {
  ...
  const allowRollback = process.env.BANK_ALLOW_STAGING_TIME_OFF_REQUESTS_ROLLBACK
    === ROLLBACK_CONFIRMATION;
  const client = new Client({ connectionString: config.connectionString, ssl: config.ssl });
  await client.connect();
  try {
    const result = await manageStagingTimeOffRequests(client, { command, allowRollback });
...
if (path.resolve(process.argv[1] || '') === path.resolve(fileURLToPath(import.meta.url))) {
  main().catch(error => {
    console.error(String(error?.message || 'Staging 0013 operation failed.')
