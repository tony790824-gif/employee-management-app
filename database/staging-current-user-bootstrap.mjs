...
const { Client } = pg;
const VERSION = '0012';
const NAME = 'current_user_bootstrap';
const LOCK_NAME = 'banke-staging-current-user-bootstrap-v1';
const ROLLBACK_CONFIRMATION = 'ROLLBACK_BANKE_STAGING_CURRENT_USER_BOOTSTRAP';

const UP_FILE = new URL('./migrations/0012_current_user_bootstrap.up.sql', import.meta.url);
const DOWN_FILE = new URL('./migrations/0012_current_user_bootstrap.down.sql', import.meta.url);

function stagingConfig(env = process.env) {
  const config = databaseTargetConfig(env);
  if (config.environment !== 'staging') {
    throw new Error('0012 current_user_bootstrap can only be managed with BANK_ENV=staging.');
  }
  return config;
}
...
async function inspect(client, checksum) {
  ...
  for (const version of ['0001', '0002', '0003', '0004', '0005', '0006', '0007', '0008', '0011']) {
  ...
  if (current && (current.name !== NAME || current.checksum !== checksum)) {
    throw new Error('Migration 0012 ledger entry does not match the approved source checksum.');
  }
  ...
  const functionDefinition = (await client.query(
    "SELECT pg_get_functiondef(to_regprocedure('app_private.api_bootstrap(text,text,text)')) AS definition"
  )).rows[0]?.definition || '';
  const currentUserContract = functionDefinition.includes("'currentUser'")
    && functionDefinition.includes("'displayName'");

  if (Boolean(current) !== Boolean(columnExists && currentUserContract)) {
    throw new Error('Migration 0012 ledger, display_name column, and currentUser contract are inconsistent.');
  }
...
export async function manageStagingCurrentUserBootstrap(client, {
  command = 'status',
  allowRollback = false
} = {}) {
  ...
  if (command === 'down' && !allowRollback) {
    throw new Error('Staging 0012 rollback requires explicit confirmation.');
  }
...
async function main() {
  ...
  const allowRollback = process.env.BANK_ALLOW_STAGING_CURRENT_USER_BOOTSTRAP_ROLLBACK
    === ROLLBACK_CONFIRMATION;
  const client = new Client({ connectionString: config.connectionString, ssl: config.ssl });
  await client.connect();
  try {
    const result = await manageStagingCurrentUserBootstrap(client, { command, allowRollback });
...
if (path.resolve(process.argv[1] || '') === path.resolve(fileURLToPath(import.meta.url))) {
  main().catch(error => {
    console.error(String(error?.message || 'Staging 0012 operation failed.')
