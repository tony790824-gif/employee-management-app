...
| M-04 | Closed in Sprint 33B and simplified on the Fast Production Path: generic Migration `status` was not strictly read-only. | `db:status:readonly` requires distinct credentials, approved host/TLS/database and one explicit `READ ONLY` transaction that reads only identity plus Migration ledger/checksums. |
