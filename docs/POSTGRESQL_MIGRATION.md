...
Before a live rehearsal, an approved public Node hosting resource must be configured with the existing Staging PostgreSQL API credential, Auth0 OIDC/JWKS settings, tenant-context signing key, exact allowed frontend origin, `BANK_ENV=staging`, and `BANK_API_BIND_HOST=0.0.0.0`. Secrets belong only in the provider secret store. Apply migration `0011_ui_bootstrap`, refresh the least-privilege role grants, then build with credential-free `BANKE_STAGING_POSTGRES_API_URL` and `BANKE_STAGING_WORKSPACE_ID` values. Never place a database credential in the browser build.
